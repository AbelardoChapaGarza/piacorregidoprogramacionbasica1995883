...
void archivos()
{
	...
	for (int i = 0; i < alta; i++)
	{
		...
		{
			archivo << "Num articulo" << VIDEOJUEGO[i].numarticulo << endl;
			archivo << "Nombre de videojuego" << VIDEOJUEGO[i].nombre.c_str() << endl;
			archivo << "a%co de salida", 164 << VIDEOJUEGO[i].año;
			archivo << "consola" << VIDEOJUEGO[i].consola.c_str() << endl;
			archivo << "clasificacion" << VIDEOJUEGO[i].clasif.c_str() << endl;
			archivo << "caracteristicas" << VIDEOJUEGO[i].caract.c_str() << endl;
			archivo << "descripcion" << VIDEOJUEGO[i].descripcion.c_str() << endl;
			archivo << "precio unitario" << VIDEOJUEGO[i].preciouni << endl;
			archivo << "impuesto" << VIDEOJUEGO[i].impuesto << endl;
			archivo << "total" << VIDEOJUEGO[i].total << endl;
		}
